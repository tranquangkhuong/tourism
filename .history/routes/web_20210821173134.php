...
Route::get('ttt', function () {
    $includes = Tour::select('values.id', 'values.attribute_id', 'values.tour_id', 'values.value')->where('tours.id', 6)->join('values', 'tours.id', '=', 'values.tour_id')->where('values.attribute_id', 2)->get()->toArray();
    foreach ($includes as $include) {
        echo '<div>' . $include->value . '</div>';
    }
});
