...
abstract class RepositoryEloquent implements RepositoryInterface
{
    ...
    public function uploadMultipleImage($hasFile, $files, $addPath = '')
    {
        if ($hasFile) {
            foreach ($files as $file) {
                $name = $file->getClientOriginalName() . '_' . time() . '.' . $file->extension();
                $file->move($this->getImageDirectory() . $addPath, $name);
            }
        }
    }
