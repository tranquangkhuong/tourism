...
<body>
    <form action="{{ route('admin.tour.plan.update', ['tour_id' => $tourId]) }}" method="post">
        ...
        <div class="container1">
            ...
            <div>
                <input type="number" name="day[]" placeholder="day" value="{{ $value['day'] }}">
                <textarea name="content[]" id="summernote" placeholder="content">{{ $value['content'] }}</textarea>
                <input type="text" name="note[]" id="" placeholder="note" value="{{ $value['note'] }}">
            </div>
            @endforeach
