...
<body>
    <form action="{{ route('admin.tour.plan.update', ['tour_id' => $tourId]) }}" method="post">
        ...
        <div class="container1">
            ...
            <div>
                <input type="number" name="day[]" placeholder="day" value="{{ $tourPlan[$key]->day }}">
                <textarea name="content[]" id="summernote" placeholder="content">{{ $key->day }}</textarea>
                <input type="text" name="note[]" id="" placeholder="note" value="{{ $key->note }}">
            </div>
            @endforeach
