...
<body>
    <form action="{{ route('admin.tour.plan.update', ['tour_id' => $tourId]) }}" method="post">
        ...
        <div class="container1">
            ...
            @foreach ($tourPlan as $key => $value)
            <div>
                <input type="number" name="day[{{ $key }}]" placeholder="day" value="{{ $value->day }}">
                <textarea name="content[{{ $key }}]" id="summernote" placeholder="content">{{ $value->day }}</textarea>
                <input type="text" name="note[{{ $key }}]" id="" placeholder="note" value="{{ $value->note }}">
            </div>
            @endforeach
